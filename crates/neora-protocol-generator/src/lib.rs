...
use neora_scanner::{
	EventOrRequestField, Interface, InterfaceChild, Protocol, ProtocolChild,
};
use proc_macro2::{Ident, Span, TokenStream};
use quote::quote;
use std::sync::LazyLock;

static PROTOCOL: LazyLock<Protocol> =
	LazyLock::new(|| neora_scanner::parse_wayland_protocol());
...
fn construct_ident_from_str_and_case(
	str: &String,
	case: Option<Case>,
) -> Ident {
	match case {
		Some(case) => match case {
			Case::CamelCase => {
				construct_indent_from_string(&str.to_upper_camel_case())
			}
			Case::SnakeCase => {
...
fn add_arg_size(arg: &neora_scanner::Arg) -> Option<TokenStream> {
	let arg_name = ident!("{}",arg.name; None);
	let arg_typ = ident!("{}", &arg.typ; Some(Case::CamelCase));
	match &arg.typ[..] {
		"String" => Some(quote! {
...
fn send_arg(arg: &neora_scanner::Arg) -> Option<TokenStream> {
	let arg_name = ident!("{}",arg.name; None);
	let arg_typ = ident!("{}", &arg.typ; Some(Case::CamelCase));
	match &arg.typ.to_upper_camel_case()[..] {
		"String" => Some(quote! {
...
fn parse_args(arg: &neora_scanner::Arg) -> Option<TokenStream> {
...
fn generate_code_for_interface(interface: &Interface) -> TokenStream {
	...
	let send_req_functions = interface.items.iter().filter_map(|msg| match msg {
        InterfaceChild::Request(req) => {
            ...
            Some(quote! {
                pub fn #function_name(&self, socket: &mut impl WaylandSocket, #(#args),*) {
                    #[allow(unused)]
                    let mut raw_size = 8;
                    #(#add_raw_size)*
                    ...
                    socket.send(&send_buffer, &send_fd);
                }
            })
        }
        _ => None
	...
	quote! {
		...
		impl WlRawObject for #struct_name {
			fn new(object_id: u32) -> #struct_name {
				#struct_name { object_id }
			}
		...
		impl #struct_name {
			fn parse_event<T>(socket: &mut T, sender_id: u32, op_code: u16, msg_body: Vec<u8>) -> Event
				where T: WaylandSocket {
				match op_code {
					#(#parse_ev)*
...
fn generate_code_for_wayland_enums() -> TokenStream {
	let enum_interface_names =
		PROTOCOL.items.iter().filter_map(|item| match item {
			ProtocolChild::Interface(interface) => {
				let interface_name =
					ident!("{}", interface.name; Some(Case::CamelCase));
				Some(quote! {#interface_name(#interface_name)})
			}
			_ => None,
		});
	let enum_event_names =
		PROTOCOL.items.iter().filter_map(|item| match item {
			ProtocolChild::Interface(interface) => {
				let event_name =
					ident!("{}Event", interface.name; Some(Case::CamelCase));
	...
	let impl_wl_get_obj = PROTOCOL.items.iter().filter_map(|item| match item {
		ProtocolChild::Interface(interface) => {
			...
			let get_function_name =
				ident!("try_get_{}", interface.name; Some(Case::SnakeCase));
			Some(quote! {
				#[allow(dead_code)]
...
pub fn generate_wayland_protocol_code() -> String {
	...
	let parse_event_for_interface = PROTOCOL.items.iter().filter_map(|item| match item {
        ProtocolChild::Interface(interface) => {
            ...
            Some(quote! {
                WlObject::#interface_name(_obj) => #interface_name::parse_event(socket, sender_id, op_code, msg_body)
            })
        }
        _ => None
    });
	...
	let code = quote! {
		...
		impl WlObject {
			pub fn parse_event<T: WaylandSocket>(&self, socket: &mut T, sender_id: u32, op_code: u16, msg_body: Vec<u8>) -> Event
				where T: {
				match self {
